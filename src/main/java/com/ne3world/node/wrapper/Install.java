...
public class Install {
    ...
    public File createDist(final WrapperConfiguration configuration) throws Exception {
        ...
        return exclusiveFileAccessManager.access(localZipFile, new Callable<File>() {
            public File call() throws Exception {
                ...
                if (installCheck.isVerified()) {
                    setExecutablePermissions(installCheck.nodeHome);
    ...
    private InstallCheck verifyDistributionRoot(File distDir, String distributionDescription)
        ...
        if (dirs.isEmpty()) {
            return InstallCheck.failure(String.format("Node distribution '%s' does not contain any directories. Expected to find exactly 1 directory.", distributionDescription));
        }
        if (dirs.size() != 1) {
            return InstallCheck.failure(String.format("Node distribution '%s' contains too many directories. Expected to find exactly 1 directory.", distributionDescription));
        }
    ...
    private void verifyDownloadChecksum(String sourceUrl, File localZipFile, String expectedSum) throws Exception {
        if (expectedSum != null) {
            // if a SHA-256 hash sum has been defined in node-wrapper.properties, verify it here
            String actualSum = calculateSha256Sum(localZipFile);
            if (!expectedSum.equals(actualSum)) {
                localZipFile.delete();
                String message = String.format("Verification of Node distribution failed!%n"
                        + "%n"
                        + "Your Node distribution may have been tampered with.%n"
                        + "Confirm that the 'distributionSha256Sum' property in your node-wrapper.properties file is correct and you are downloading the wrapper from a trusted source.%n"
                        + "%n"
    ...
    private void setExecutablePermissions(File nodeHome) {
        if (isWindows()) {
            return;
        }
        File nodeCommand = new File(nodeHome, "bin/node");
        String errorMessage = null;
        try {
            ProcessBuilder pb = new ProcessBuilder("chmod", "755", nodeCommand.getCanonicalPath());
            Process p = pb.start();
            if (p.waitFor() != 0) {
        ...
        if (errorMessage != null) {
            logger.log("Could not set executable permissions for: " + nodeCommand.getAbsolutePath());
        }
    }
    ...
    private static class InstallCheck {
        ...
        private static InstallCheck success(File nodeHome) {
            return new InstallCheck(nodeHome, null);
        }

        private InstallCheck(File nodeHome, String failureMessage) {
            this.nodeHome = nodeHome;
            this.failureMessage = failureMessage;
        }

        private boolean isVerified() {
            return nodeHome != null;
        }
    }
